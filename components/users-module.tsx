...
type User = { id: string; name: string; alias: string; email: string; role: string; isActive: boolean; lastLoginAt: string | null; createdAt: string }
...
export default function UsersModule() {
  ...
  return <div className="dashboard-content">
    ...
    <article className="panel module-table"><div className="table-scroll"><table><thead><tr><th>Usuario</th><th>Alias</th><th>Correo</th><th>Rol</th><th>Se unió</th><th>Último acceso</th><th>Estado</th><th /></tr></thead><tbody>{users.map((user) => <tr key={user.id}><td><div className="product-cell"><span className="product-thumb"><UserRound size={16} /></span><strong>{user.name}</strong></div></td><td>{user.alias}</td><td>{user.email}</td><td>{roleLabels[user.role] ?? user.role}</td><td>{new Date(user.createdAt).toLocaleString('es-BO')}</td><td>{user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString('es-BO') : 'Nunca'}</td><td><span className={`status ${user.isActive ? 'success' : 'warning'}`}><i />{user.isActive ? 'Activo' : 'Inactivo'}</span></td><td><button className="outline-button" onClick={() => void toggle(user)}>{user.isActive ? 'Desactivar' : 'Activar'}</button></td></tr>)}</tbody></table></div></article>
  </div>
}
