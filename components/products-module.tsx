...
export default function ProductsModule() {
  ...
  async function handleDeleteProduct(id: string) {
    ...
    setMessage(response.ok ? `Producto eliminado: ${data.name ?? 'ok'}` : data.error ?? 'No se pudo eliminar el producto.')
  ...
  async function handleDeleteCategory(id: string) {
    ...
    const data = await response.json().catch(() => ({}))
    setMessage(response.ok ? `Categoría eliminada: ${data.name ?? 'ok'}` : data.error ?? 'No se pudo eliminar la categoría.')
    if (response.ok) {
      setCategories((current) => current.filter((category) => category.id !== id))
      setForm((current) => ({ ...current, categoryId: current.categoryId === id ? '' : current.categoryId }))
      await loadProducts()
    }
  }
  ...
  return <div className="dashboard-content">
    ...
    {categories.length > 0 && <div className="panel"><div className="table-scroll"><table><thead><tr><th>Categoría</th><th>Acción</th></tr></thead><tbody>{categories.map((category) => <tr key={category.id}><td>{category.name}</td><td><button className="outline-button" onClick={() => void handleDeleteCategory(category.id)}>Eliminar</button></td></tr>)}</tbody></table></div></div>}
